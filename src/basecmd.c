...
struct move_node {
    uint32_t stepper1_steps;
    uint32_t stepper2_steps;
    uint32_t stepper3_steps;
    uint32_t stepper4_steps;
    uint32_t stepper5_steps;
    uint32_t stepper6_steps;

    uint32_t stepper1_speed;
    uint32_t stepper2_speed;
    uint32_t stepper3_speed;
    uint32_t stepper4_speed;
    uint32_t stepper5_speed;
    uint32_t stepper6_speed;
...
get_oid()
{
	sendf("oid oid=%c type=%p data=%p", READP(command_get_oid_oid)
		  , oid_lookup(READP(command_get_oid_oid), NULL), NULL);
}
DECL_COMMAND_FLAGS(get_oid, HF_IN_SHUTDOWN, "get_oid oid=%c");
...
void command_raw_move(uint32_t *args) {
    struct move_node *move = move_alloc();
    if (!move) {
       print("Move queue overflow");
       sendf("full");
       return;
    }

    // Assign the steps and speeds to each stepper
    move->stepper1_steps = args[0];
    move->stepper2_steps = args[1];
    move->stepper3_steps = args[2];
    move->stepper4_steps = args[3];
    move->stepper5_steps = args[4];
    move->stepper6_steps = args[5];

    move->stepper1_speed = args[6];
    move->stepper2_speed = args[7];
    move->stepper3_speed = args[8];
    move->stepper4_speed = args[9];
    move->stepper5_speed = args[10];
    move->stepper6_speed = args[11];

    // Add the move to the queue
    struct move_queue_head *mh = &some_global_move_queue;  // Replace with actual move queue reference
    move_queue_push(move, mh);
}

DECL_COMMAND(command_raw_move, "raw_move s1_steps=%u s2_steps=%u s3_steps=%u s4_steps=%u s5_steps=%u s6_steps=%u s1_speed=%u s2_speed=%u s3_speed=%u s4_speed=%u s5_speed=%u s6_speed=%u");


 void command_move_stepper(uint8_t stepper, int8_t direction, uint8_t speed, uint32_t steps) {
    // Implement logic to move the specified stepper motor
    // based on the direction, speed, and steps parameters.
}
DECL_COMMAND(command_move_stepper, "move_stepper stepper=%c direction=%c speed=%c steps=%u");
