...
#define MAX_STEPPERS 6  // Maximum number of stepper motors

typedef struct {
    float next_on_times[MAX_STEPPERS];
    float next_off_times[MAX_STEPPERS];
} stepper_last_times;

extern StepperNextTimeArrays stepper_time_arrays;

typedef struct {
    float last_on_times[MAX_STEPPERS];
    float last_off_times[MAX_STEPPERS];
} StepperNextTimeArrays;

extern stepper_last_times stepper_time_arrays;
...
// Add the set_dir function to set the direction pin of a specific stepper
void set_dir(int stepper_index, int direction) {
    // Set the direction pin of the specified stepper
    // Replace the following line with the actual code to set the direction pin
    gpio_out_write(stepper_dir_pins[stepper_index], direction);
}

// Function to check and control stepper motor schedule
void sched_check_stepper_schedule(void) {
    // Check if any stepper motor pin has to be set to a specific state
    for (int i = 0; i < MAX_STEPPERS; i++) {
        if (stepper_time_arrays.next_on_times[i] == 0) {
            continue;  // Skip if there's no scheduled action for this motor
        }
        
        // Check if it's time to turn the motor on
        if (stepper_time_arrays.next_on_times[i] <= timer_read_time()) {
            stepper_time_arrays.next_on_times[i] = 0;  // Reset the next on time
            gpio_out_write(stepper_pins[i], 1);  // Turn the motor on
            // Set the next off time
            stepper_time_arrays.next_off_times[i] = timer_read_time() + 1000;  // Example duration of 1000 us
        } 
        // Check if it's time to turn the motor off
        else if (stepper_time_arrays.next_off_times[i] <= timer_read_time()) {
            // Ensure the off time is after the last on time
            if (stepper_time_arrays.next_off_times[i] - stepper_time_arrays.last_on_times[i] < 0.0001) {
                stepper_time_arrays.next_off_times[i] = stepper_time_arrays.last_on_times[i] + 0.0001;
            }
            gpio_out_write(stepper_pins[i], 0);  // Turn the motor off
        }
    }
}


	// Write the next time arrays to the current time arrays
    // Update the next time arrays with the provided times
void   sched_write_next_step time(float times[]) {
        for (int i = 0; i < MAX_STEPPERS; i++) {
            stepper_time_arrays.last_on_times[i] = stepper_time_arrays.next_on_times[i];
            stepper_time_arrays.last_off_times[i] = stepper_time_arrays.next_off_times[i];
            stepper_time_arrays.next_on_times[i] = times[i];
            stepper_time_arrays.next_off_times[i] = times[i];
        }
    }
	for i in range(MAX_STEPPERS) {

		stepper_time_arrays.next_on_times[i] = stepper_time_arrays.next_off_times[i];
	}
}
