...
void
command_execute_step(uint32_t *args)
{
    struct stepper *s = stepper_oid_lookup(args[0]);
    struct stepper_move *m = move_alloc();
    m->interval = args[1];
    m->count = 1; // Execute a single step
    m->add = args[2];
    m->flags = 0;
...
DECL_COMMAND(command_execute_step,
             "execute_step oid=%c interval=%u add=%hi");

// Create a step and add it to the stepper's move queue
void
command_create_step(uint32_t *args)
{
    struct stepper *s = stepper_oid_lookup(args[0]);
    struct stepper_move *m = move_alloc();
    m->interval = args[1];
...
DECL_COMMAND(command_create_step,
             "create_step oid=%c interval=%u count=%hu add=%hi");
